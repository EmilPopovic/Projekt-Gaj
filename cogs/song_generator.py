...
from spotify import SpotifyInfo, SpotifySong, Author
...
class SongGenerator:
    ...
    def set_color(self):
        ...
        # dominant_color = self.get_thumbnail_color()
        # # convert to discord-friendly format
        # self.color = discord.Color.from_rgb(*dominant_color)
        self.color = 0xf1c40f

    def get_thumbnail_color(self) -> tuple:
        """
        Returns the dominant color of song thumbnail.
        Formatted as (r, g, b).
        """
        # get image from url
    ...
    def multithread_extract(songs: list[SpotifySong]):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            return sorted([song for song in executor.map(SongGenerator, songs)])
